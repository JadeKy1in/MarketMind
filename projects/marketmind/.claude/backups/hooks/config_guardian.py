...
# ---------------------------------------------------------------------------
# Guard 5: mandatory enforcement hooks — protect all workflow hook entries
# ---------------------------------------------------------------------------

# Required hook entries that MUST be present in settings.local.json
# Format: event → [list of hook script names that must be wired]
REQUIRED_HOOK_ENTRIES = {
    "SessionStart": [
        "integrity_check.py",
        "config_guardian.py",
        "time_anchor.py",
...
HOOKS_DIR = Path("E:/AI_Studio_Workspace/.claude/hooks")

# Template hook entries for restoration — keyed by (event, script)
HOOK_ENTRY_TEMPLATES = {
    ("SessionStart", "integrity_check.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/integrity_check.py\""}
        ],
    },
    ("SessionStart", "config_guardian.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/config_guardian.py\""}
        ],
    },
    ("SessionStart", "time_anchor.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/time_anchor.py\""}
        ],
    },
    ("SessionStart", "task_manifest.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/task_manifest.py\""}
        ],
    },
    ("SessionStart", "startup_report.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/startup_report.py\""}
        ],
    },
    ("PreToolUse", "danger_guard.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/danger_guard.py\""}
        ],
    },
    ("Stop", "stop_gate_check.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/stop_gate_check.py\""}
        ],
    },
    ("Stop", "conversation_archiver.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/conversation_archiver.py\""}
        ],
    },
    ("PreCompact", "pre_compact.py"): {
        "matcher": "",
        "hooks": [
            {"type": "command", "command": "python \"E:/AI_Studio_Workspace/.claude/hooks/pre_compact.py\""}
        ],
    },
}


def guard5_enforcement_hooks(settings):
    """Ensure all mandatory workflow enforcement hook entries exist.

    Checks SessionStart, PreToolUse, Stop, and PreCompact hook lists
    for required entries. Restores any missing entries from hardcoded templates.
    """
    if "hooks" not in settings:
        settings["hooks"] = {}
    ...
    for event, required_scripts in REQUIRED_HOOK_ENTRIES.items():
        ...
        # Find which scripts are already wired
        wired_scripts = set()
        ...
        for script in required_scripts:
            if script not in wired_scripts:
                template_key = (event, script)
                template = HOOK_ENTRY_TEMPLATES.get(template_key)
                if template:
                    import copy
                    event_hooks.append(copy.deepcopy(template))
                    restored.append(f"{event}:{script}")

    if restored:
        log_recovery("hooks", "restored", f"enforcement hooks: {', '.join(restored)}")
        return True
    else:
        log_ok("hooks (enforcement entries)")
        return False
...
def main():
    ...
    if guard5_enforcement_hooks(settings):
        needs_write = True
