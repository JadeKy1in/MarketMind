...
**版本**: 4.0 Final
**日期**: 2026-05-21 (更新：新增 §9.6 日内交易微观结构研究)
**状态**: 权威参考文档 —— 取代所有此前碎片化文档
**整合来源**: 7份设计文档 + 5份研究Batch 1报告 + 3份研究Batch 2报告 + 外部GitHub探索 + 日内研究 + 日内微观结构研究 (2026-05-21)
...
> **研究日期**: 2026-05-21 | **状态**: 写入最终方案 | **受益影子**: Intraday Scalper (日内猎手)

#### 9.6.1 日内市场微观结构基础

日内交易依赖三个核心微观结构概念，这些是 Intraday Scalper 影子策略设计的理论基础：

**买卖价差 (Bid-Ask Spread)**:
- 价差 = 流动性成本。标普 500 成分股平均价差 ~0.01-0.03%（2026 数据），小盘股 ~0.10-0.30%
- 价差呈现 U 形日内模式：开盘宽（信息不对称最高）→ 午间收窄 → 收盘放宽（库存风险）
- Intraday Scalper 入场应避免开盘前 15 分钟（价差最宽），最优入场窗口为开盘后 30-60 分钟

**市场深度 (Order Book Depth)**:
- Level 2 数据（订单簿前 5-10 档）反映真实供需
- 大单挂在远离最优价的位置 = "冰山订单"，表明机构正在吸筹/派发
- 影子可通过 VWAP（成交量加权均价）偏差检测机构行为：价格持续高于 VWAP = 买方主导

**信息不对称周期**:
```
开盘 (09:30-10:00): 隔夜信息消化期 — 波动高，方向噪音多，价差宽
早盘 (10:00-11:30): 机构调仓期 — 方向开始清晰，成交量稳定
午间 (11:30-14:00): 流动性低谷 — 成交量下降 30-40%，价格易被操纵
尾盘 (14:00-15:30): 收盘驱动期 — ETF 再平衡、做市商对冲，成交量回升
收盘 (15:30-16:00): 竞价期 — 成交量暴增，方向信息含量最高
```

#### 9.6.2 日内波动率模式

| 时段 | 波动率水平 | 成交量 | 方向可靠性 | 交易建议 |
|------|:--:|:--:|:--:|------|
| 09:30-10:00 | 最高 (2-3× 日均) | 最高 | 低（噪音主导） | **观察期** — 记录开盘区间高低点 |
| 10:00-11:30 | 中等 | 中高 | 中等 | 开盘区间突破入场（最可靠信号） |
| 11:30-14:00 | 低 (0.6-0.8× 日均) | 最低 | 低（假突破多） | **禁止新开仓** — 仅管理现有持仓 |
| 14:00-15:30 | 中等偏高 | 中高 | 高 | 趋势延续/反转入场 |
| 15:30-16:00 | 高 | 最高 | 高 | 收盘前平仓（不过夜） |

**波动率微笑**: 日内波动率呈 U 形（开盘高→午间低→收盘高），与美国市场研究 (Andersen & Bollerslev, 1997) 一致。

#### 9.6.3 开盘区间突破策略（ORB）

这是 Intraday Scalper 的核心策略框架，基于 Opening Range Breakout：

```
ORB 定义:
  - 开盘 30 分钟的最高价 = OR_High
  - 开盘 30 分钟的最低价 = OR_Low
  - 开盘区间宽度 = OR_High - OR_Low

入场信号:
  - 价格突破 OR_High + 0.2×区间宽度 → LONG
  - 价格跌破 OR_Low - 0.2×区间宽度 → SHORT
  - 成交量需 > 前 5 日均量 50%（确认突破有效）

过滤条件（避免假突破）:
  - 开盘区间宽度 > ATR(14) × 0.3（区间太窄 = 无方向共识，跳过）
  - VIX > 35（恐慌市场开盘噪音极大，ORB 失效，跳过）
  - 重大数据发布日 (FOMC, CPI, NFP) — 跳过开盘交易，等待波动稳定
```

**学术基础**: Crabel (1990) 原始 ORB 研究 + Gao & Han (2021) 基于机器学习的 ORB 优化。Gao et al. (2018) 发现 30 分钟区间对美国市场最优（平衡信息吸收与过早入场风险）。

#### 9.6.4 日内成交量分析

**相对成交量 (Relative Volume, RVOL)**:
```
RVOL = 当前 5 分钟成交量 / 前 20 日同时段均值
RVOL > 2.0 = 异常活跃 → 潜在信号
RVOL < 0.5 = 流动性枯竭 → 避免交易
```

**成交量加权均价 (VWAP)**:
- VWAP = Σ(价格 × 成交量) / Σ成交量（当日累计）
- 价格 > VWAP = 日内买方主导；价格 < VWAP = 日内卖方主导
- VWAP 回归交易：极端偏离 VWAP (>2σ) 后回归是高频策略核心
- Intraday Scalper 的目标持仓期为 1-3 天，VWAP 偏差用于判断日内入场时机

**成交量形态分类**:
| 形态 | 特征 | 含义 | Scalper 动作 |
|------|------|------|------|
| 递增放量 | 成交量逐步放大 + 价格同向 | 趋势确认 | 顺势入场 |
| 天量滞涨 | 巨量但价格不动 | 派发/吸筹 | 逆向准备 |
| 缩量反弹 | 反弹时成交量萎缩 | 弱势反弹 | 反弹做空 |
| 放量下跌后缩量 | 恐慌释放后缩量 | 抛压衰竭 | 反向做多 |

#### 9.6.5 中美市场日内差异

Intraday Scalper 当前仅覆盖美国市场（yfinance 支持）。如果未来扩展到中国市场（通过 AKShare），需注意以下关键差异：

| 维度 | 美国市场 | 中国 A 股市场 |
|------|------|------|
| 交易时间 | 09:30-16:00 EST 连续 | 09:30-11:30 + 13:00-15:00 CST 分段 |
| 午间缺口 | 无（连续交易） | **关键** — 午休 1.5h 产生跳空，下午开盘首 15 分钟波动极大 |
| 涨跌停限制 | 无（有熔断） | ±10%（主板）/ ±20%（科创/创业）— 流动性陷阱 |
| T+0 | 允许 | **禁止**（T+1）— 日内影子需虚拟化处理 |
| 做空机制 | 广泛可用 | 仅通过融券（成本高、券源有限） |
| 开盘竞价 | 09:30 连续竞价 | 09:15-09:25 集合竞价 → 09:30 连续 |
| 散户占比 | ~20% | ~70% — 更高噪音，更多情绪驱动 |
| 流动性日内模式 | U 形 | W 形（上午开盘 + 下午开盘两个高峰） |

**中国市场的特殊考量**:
- T+1 制度意味着"日内"影子在中国市场实际是"隔日"交易
- 涨停/跌停板可能导致流动性枯竭（买不到/卖不掉）
- 午间新闻消化在下午开盘集中爆发，波动模式不同于美国
- 中国市场扩展时，Intraday Scalper 需要一个独立的、针对中国市场的配置变量集

#### 9.6.6 数据基础设施需求

**当前状态**:
- [x] yfinance 日线数据（免费，延迟 15 分钟）
- [ ] **日内分钟级数据** — **缺失**

**Phase 1 — Polygon.io（推荐主源）**:
```
端点: https://api.polygon.io/v2/aggs/ticker/AAPL/range/1/minute/2026-05-21/2026-05-21
免费 tier: 5 调用/分钟（足够 Intraday Scalper 每日分析 5 个标的）
成本: $0 (free tier), $29/mo (Basic), $79/mo (Starter)
覆盖: 美股 + ETF，实时数据（Basic+ 计划）
```

**Phase 2 — Alpha Vantage（免费备源）**:
```
端点: https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=IBM&interval=5min&apikey=KEY
免费 tier: 25 调用/天
局限: 仅 5/15/30/60min 间隔，无 1min 数据
```

**数据存储策略**:
- 日内数据按需拉取（不预先存储全部资产的分钟数据 — 成本过高）
- Intraday Scalper 在每次双时间点检查时拉取目标标的最新分钟数据
- 每次拉取缓存至 SQLite `intraday_cache` 表（TTL=1 天）
- 每日仅需 2-4 次 API 调用（开盘检查 1-2 次 + 收盘检查 1-2 次）

**回测数据**: 3 个月历史分钟数据用于策略回测（~200MB 压缩存储），来源：Databento（$125 免费额度）或 Polygon Flat Files。

#### 9.6.7 Intraday Scalper 全天操作时间线

```
09:30 开盘
  ├── [观察] 不交易 — 记录开盘区间
  │
10:00 双时间点检查 #1: 开盘区间突破信号
  ├── [计算] ORB 上下边界 + 成交量确认
  ├── [决策] 是否入场？方向？仓位？
  ├── [数据] 拉取过去 30 分钟的 1 分钟 OHLCV (Polygon)
  │   成本: 1 API 调用/标的
  ├── [LLM] 仅当 ORB 触发信号时调用 Pro
  │   输入: 开盘区间数据 + 成交量形态 + 昨日新闻摘要 (~800 tokens)
  │   输出: 入场方向 + 目标价位 + 止损价位 (~150 tokens)
  │   成本: ~$0.001/调用 (DeepSeek Pro)
  │
11:30-14:00 午间
  └── [禁止] 不新开仓，仅监控已有持仓
  │
14:00-15:00 尾盘评估
  ├── [监控] 已有持仓是否需要平仓/调整
  │
15:30 双时间点检查 #2: 收盘前操作
  ├── [强制] 所有日内仓位必须在此窗口平仓 (T+0)
  ├── [LLM] 仅当有未平仓位时调用 Pro
  │   输入: 当日走势 + 持仓状态 + 明日预判 (~500 tokens)
  │   输出: 平仓/持有过夜决策 (~100 tokens)
  │   注意: 持有过夜仅限动量极强（连续 3 根 30 分钟阳线 + RVOL>2.0）
  │
16:00 收盘
  └── [归档] 当日交易记录写入 virtual_trades
```

**每日成本预估**:
- API 数据拉取: 2-4 次 Polygon 调用（免费 tier 覆盖）
- LLM 调用: 0-2 次 Pro（仅信号触发时调用，非信号日 $0）
- 月度总增量: ~$1-3（远低于原始 Pro 全量分析的 $30-60/月）

#### 9.6.8 日内风险控制

**Intraday Scalper 特有风控（叠加中央风控）**:

| 规则 | 阈值 | 违规动作 |
|------|:--:|------|
| 单日最大交易次数 | 3 次 | 第 4 个信号自动拒绝 |
| 单笔最大亏损 | -2% 名义本金 | 硬止损（非心理止损） |
| 单日最大亏损 | -5% 名义本金 | 全天停止交易 |
| 连续亏损交易 | 3 笔 | 暂停 1 天（强制冷却） |
| 午间持仓 | 仅限管理 | 禁止 11:30-14:00 新开仓 |
| 重大数据日 | 跳过开盘交易 | FOMC/CPI/NFP 日 — ORB 信号不触发 |
| 隔夜持仓上限 | ≤ 资本 10% | 收盘前超标仓位强制平仓 |

**与中央风控的关系**: 日内影子虚拟交易 → 不影响真实账户。风控规则用于排名评估（频繁违规的影子惩罚性降级）。
...
- Gao, Y. & Han, X. (2021). Opening range breakout with machine learning. *SSRN*.
- Cont, R. (2001). Empirical properties of asset returns: stylized facts and statistical issues. *Quantitative Finance*.
- Biais, B., Hillion, P., & Spatt, C. (1995). An empirical analysis of the limit order book and the order flow in the Paris Bourse. *Journal of Finance*.
