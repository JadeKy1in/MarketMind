...
1. 读取个人记忆（过往决策、成功、失败）
2. 读取今日原始新闻/事实
3. 观察用户+主AI讨论（用户意见+提交材料）——**但不看主AI的分析报告**（避免锚定偏差）
4. 初步分析
5. 使用 Flash 配额请求额外数据（迭代，影子自主决定）
6. 判断是否继续深入还是做出决定
7. 投资决策
...
| 影子接收 ✅ | 影子不接收 ❌ |
|------|------|
| Stage 1 采集的全部原始新闻 | 主 AI 在 Stage 3 的叙事分析 |
| 市场数据（yfinance/Finnhub/Binance） | 主 AI 在 Stage 4 的 L2/L3 结论 |
| 用户原始意见和材料 | 其他影子在同周期的分析输出 |
| 宏观数据（FRED/EIA/CFTC） | 主 AI 的预讨论报告 |

### 时间安排

影子不"等待"主 AI 完成——它们在 Stage 0 初始化后，在 Stage 4（L2+L3）的同一时间运行自己的分析。ELITE 影子在 Gate 2 被唤醒时，贡献的是它们并行分析的**预计算结果**，不是事后补的。
